...
//   q.last = (q.last + 1) % q.qsize;memcpy(&(q.q[q.last]), &ind, sizeof(Interval));q.count = q.count + 1;
  ...
  while(!empty(&q)) {
    ...
    //res = q.q[q.first];q.first = (q.first + 1) % q.qsize;q.count = q.count - 1;
...
void kd_print(Kdtree *tree) {
  ...
  for(i = 0; i < tree->size; i++) {
    printf("%d (%.2f, %.2f, %.2f) (%.2f, %.2f) ", i, tree->nodes[i].coords[0], tree->nodes[i].coords[1], 
           tree->nodes[i].coords[2], tree->nodes[i].v, tree->nodes[i].p);
    printf("neighbors: ");
